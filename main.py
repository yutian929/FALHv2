...
class RoomStateBarManager:
    ...
    def _init_bars(self):
        num_bars = int(np.ceil(self.total_perimeter / BAR_LENGTH))
        for i in range(num_bars):
            start_dist = i * BAR_LENGTH
            # 处理最后一个 Bar 可能不足 BAR_LENGTH 的情况
            actual_len = min(BAR_LENGTH, self.total_perimeter - start_dist)
        ...
        for bar in self.bars:
            ...
            if ratio > 0: # 只要看到了
                if bar.is_empty:
                    ...
                    should_update = True
                elif ratio > THRESHOLD_BAR_UPDATE:
                    # 情况2: 已经有信息，需要交并比大于阈值
                    should_update = True
    ...
    def visualize(self, cam_mat=None):
        """
        可视化当前房间状态。
...
if __name__ == "__main__":
    ...
    manager = RoomStateBarManager(ROOM_POINTS)
    ...
    for i, traj_path in enumerate(traj_files):
        ...
        if i % 10 == 0: # 每10帧打印一次进度
            manager.visualize(cam_mat=cam_mat)
            print(f"Processing Frame {idx}... Updated Bars: {len(updated)}")
